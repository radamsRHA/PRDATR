#' Function_ComputePairwiseDistances_SimulateTips_Aldous2: function to generate pairwise distance computations
#'
...
##############################################################
# Function_ComputePairwiseDistances_SimulateTips_Aldous2 #
##############################################################
Function_ComputePairwiseDistances_SimulateTips_Aldous2 <- function(numeric.NumberReps, numeric.n, string.Model, vector.Theta){
  ...
  for (t in 1:numeric.NumberReps){
    ...
    handle.Tree <- as.phylo.treeshape(x = raldous(tip.number = numeric.n)[[1]])
    handle.Tree$edge.length <- rexp(n = length(handle.Tree$edge.length), rate = 100)
    #handle.Tree$edge.length <- rep(1, length(handle.Tree$edge.length))
    #handle.Tree <- force.ultrametric(tree = handle.Tree, method = "extend")
    handle.Tree <- chronos(handle.Tree, quiet = T)
